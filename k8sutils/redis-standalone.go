...
func CreateStandaloneService(cr *redisv1beta1.Redis) error {
	...
	annotations := generateServiceAnots(cr.ObjectMeta, cr.Spec.KubernetesConfig.Service)
	...
	err := CreateOrUpdateService(cr.Namespace, headlessObjectMetaInfo, redisAsOwner(cr), false, true, cr.Spec.KubernetesConfig.Service.ServiceType)
	if err != nil {
		logger.Error(err, "Cannot create standalone headless service for Redis")
		return err
	}
	err = CreateOrUpdateService(cr.Namespace, objectMetaInfo, redisAsOwner(cr), enableMetrics, false, cr.Spec.KubernetesConfig.Service.ServiceType)
	if err != nil {
