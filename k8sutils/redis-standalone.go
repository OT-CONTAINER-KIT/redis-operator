...
func CreateStandaloneService(cr *redisv1beta1.Redis) error {
	...
	err := CreateOrUpdateService(cr.Namespace, headlessObjectMetaInfo, redisAsOwner(cr), false, true, cr.Spec.KubernetesConfig.ServiceType)
	if err != nil {
		logger.Error(err, "Cannot create standalone headless service for Redis")
		return err
	}
	err = CreateOrUpdateService(cr.Namespace, objectMetaInfo, redisAsOwner(cr), enableMetrics, false, cr.Spec.KubernetesConfig.ServiceType)
	if err != nil {
