...
# Preserve packaging Dockerfile from the start branch — older tags (e.g. v0.22.2)
# do not contain tests/e2e-rejoin/, so copying it after checkout would fail.
DOCKERFILE_PKG="$(mktemp)"
cp "$REPO/tests/e2e-rejoin/Dockerfile.package" "$DOCKERFILE_PKG"
trap 'rm -f "$DOCKERFILE_PKG"' EXIT
...
build() { # $1 branch  $2 tag
  ...
  local W; W=$(mktemp -d); cp /tmp/op-noskip "$W/operator"; cp "$DOCKERFILE_PKG" "$W/Dockerfile"
...
build fix/reattach-misplaced-replicas fix-reattach-noskip
