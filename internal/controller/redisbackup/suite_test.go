...
func TestAPIs(t *testing.T) {
	...
	RunSpecs(t, "RedisBackup Controller Suite")
}
...
var _ = BeforeSuite(func() {
	...
	err = (&Reconciler{
		...
		RESTConfig: cfg,
		// envtest has no kubelet and no S3, so the real BGSAVE/copy/upload
		// pipeline cannot run here. Stub it out and let the specs cover the
		// validation, phase transitions and status bookkeeping instead; the
		// real pipeline is exercised by the e2e suite.
		backupFn: func(_ context.Context, instance *redisv1alpha1.RedisBackup) (string, error) {
			return fmt.Sprintf("s3://%s/backups/%s/%s",
				instance.Spec.S3.Bucket,
				instance.Spec.RedisClusterName,
				time.Now().UTC().Format("2006-01-02T15-04-05Z")), nil
		},
