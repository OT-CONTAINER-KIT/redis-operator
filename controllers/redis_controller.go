...
func (r *RedisReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		if instance.Spec.Mode == "cluster" {
			...
			if masterReplicas == nil {
				masterReplicas = instance.Spec.Size
			}
			slaveReplicas := instance.Spec.Slave.Size
			if slaveReplicas == nil {
				slaveReplicas = instance.Spec.Size
			}
			totalReplicas := int(*masterReplicas) + int(*slaveReplicas)

			if int(redisMasterInfo.Status.ReadyReplicas) != int(*masterReplicas) && int(redisSlaveInfo.Status.ReadyReplicas) != int(*slaveReplicas) {
				reqLogger.Info(
					"Redis master and slave nodes are not ready yet",
					"Master.Ready.Replicas", strconv.Itoa(int(redisMasterInfo.Status.ReadyReplicas)),
					"Slave.Ready.Replicas", strconv.Itoa(int(redisSlaveInfo.Status.ReadyReplicas)),
				)
			...
			if k8sutils.CheckRedisNodeCount(instance, "") != totalReplicas {
				...
				if masterCount != int(*masterReplicas) {
					reqLogger.Info("Not all masters are part of the cluster...", "Masters.Count", masterCount, "Master.Replicas", masterReplicas)
					k8sutils.ExecuteRedisClusterCommand(instance)
				} else {
					reqLogger.Info("All masters are part of the cluster, adding slaves/replicas", "Masters.Count", masterCount, "Master.Replicas", masterReplicas)
					k8sutils.ExecuteRedisReplicationCommand(instance)
				}
			} else {
				reqLogger.Info("Redis master count is desired")
				if k8sutils.CheckRedisClusterState(instance) >= totalReplicas-1 {
